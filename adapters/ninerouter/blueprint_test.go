...
func TestBlueprintStoresTheFullDocument(t *testing.T) {
	...
	if first.CoreConcept == "" || len(first.KeyPoints) != 3 ||
...
func TestBlueprintPromptCarriesTheBudget(t *testing.T) {
	...
	// 45 chapters at the 450-word default is 20250 words, 155 minutes at 130 wpm.
	if _, err := c.Blueprint(context.Background(), testRequest(45)); err != nil {
		t.Fatalf("Blueprint: %v", err)
	}
...
// Without a target duration the budget falls out of the chapter count at the
// default chapter size, which is how videos planned before durations existed
// keep the shape they were planned with.
func TestBlueprintBudgetsFromTheChapterCount(t *testing.T) {
	t.Parallel()
	g := newGateway(t, http.StatusOK, completion(outline(1)))
	c := newClient(t, g, "")

	if _, err := c.Blueprint(context.Background(), testRequest(10)); err != nil {
		t.Fatalf("Blueprint: %v", err)
	}
...
// A target duration is the honest input when you want three hours and do not
// much mind how many chapters that takes.
