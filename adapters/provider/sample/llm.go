...
// LLM writes real JSON and text assets to the store and returns their content
// addresses.
type LLM struct {
...
func (l *LLM) Blueprint(ctx context.Context, req provider.BlueprintRequest) (provider.Blueprint, error) {
	...
	for i := 1; i <= req.ChapterCount; i++ {
		...
		bp.Chapters = append(bp.Chapters, provider.BlueprintChapter{
			...
			EstimatedWords: chapterWords(req),
		})
	}
...
// chapterWords is the flat per-chapter budget this backend plans with. It does
...
func chapterWords(provider.BlueprintRequest) int { return entity.DefaultWordsPerChapter }
...
func (l *LLM) SlidePrompts(ctx context.Context, videoID entity.VideoID) ([]provider.SlidePrompt, error) {
	...
	if !ok {
		return nil, fmt.Errorf("sample provider: unexpected prompt cache type %T", v)
	}
...
func (l *LLM) Metadata(ctx context.Context, req provider.MetadataRequest) (provider.Metadata, error) {
	...
	md := entity.Metadata{
		...
		// The hook is what the thumbnail is built around, so this writes one
...
func (l *LLM) ThumbnailPlan(ctx context.Context, req provider.ThumbnailPlanRequest) (provider.ThumbnailPlan, error) {
	if req.Cells < 1 {
		return provider.ThumbnailPlan{}, errors.New("sample llm: a thumbnail plan needs at least one cell")
	}
	if len(req.Blueprint.Chapters) == 0 {
		return provider.ThumbnailPlan{}, errors.New("sample llm: a thumbnail plan needs an outline to draw from")
	}
