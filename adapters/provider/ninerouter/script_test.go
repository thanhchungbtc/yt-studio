...
func TestScriptFallsBackWhenNoBudgetIsAssigned(t *testing.T) {
	t.Parallel()
	t.Run("the outline entry answers", func(t *testing.T) {
		t.Parallel()
		g := newGateway(t, http.StatusOK, completion(narration))
		req := scriptRequest()
		req.TargetWords = 0
		if _, err := newClient(t, g, "").Script(context.Background(), req); err != nil {
			t.Fatalf("Script: %v", err)
		}
		if user := g.messages(t)["user"]; !strings.Contains(user, "about 280 words") {
			t.Errorf("the outline entry's budget did not answer:\n%s", user)
		}
	})

	// Nothing assigned a budget anywhere, so the default is all that is left.
	t.Run("nothing assigned one", func(t *testing.T) {
		t.Parallel()
		g := newGateway(t, http.StatusOK, completion(narration))
		req := scriptRequest()
		req.TargetWords = 0
		...
		want := "about " + strconv.Itoa(entity.DefaultWordsPerChapter) + " words"
