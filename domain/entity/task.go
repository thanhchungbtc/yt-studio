...
// TaskKind identifies a node type in the per-video DAG (§4).
...
const (
	...
	// chapter's image prompts in one call, so there is a batch to coalesce (§4).
...
// Pool returns the single pool a task of this kind acquires a slot in (§5).
...
// PerChapter reports whether the DAG contains one node of this kind per
// chapter (as opposed to one per video).
...
const (
	...
	// TaskStateAwaitingApproval succeeded but sits on a gate; it has not
	// released its dependents and consumes nothing (§6).
...
// GateKind names a human approval gate (§6).
...
// may restart freely while a gate is open (§6).
type Task struct {
	ID      TaskID
	VideoID VideoID
	...
	Ordinal int
	// Index distinguishes siblings of the same kind for one chapter — the
	// image index within a chapter — or -1 when there is only one.
	Index int

	State TaskState
	Pool  Pool
	// Gate, when set, means: on success, park in awaiting_approval and do not
	// release dependents until a human approves (§6).
	Gate GateKind
...
// package can add a case (§8.2). Every type switch over it must end with a
// default that panics, and a table-driven test asserts each site handles every
// case.
...
// exhaustiveness tests iterate it against every type-switch site (§8.2).
