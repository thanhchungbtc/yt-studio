...
func TestNewVideoValidation(t *testing.T) {
	t.Parallel()
	now := time.Unix(0, 0)
	if _, err := entity.NewVideo("v1", "c1", "DSS-1", "Title", "topic", 50, 2, now); err != nil {
	...
	for _, tc := range bad {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := entity.NewVideo("v1", "c1", tc.ref, tc.title, "", tc.chapters, tc.images, now); !errors.Is(err, entity.ErrInvalidVideo) {
