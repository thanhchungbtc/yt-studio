...
type StoredAsset struct {
	// ID is the sha256 of the bytes: the content address (§3).
	ID entity.AssetID
	// Path is relative to the store root, so the store can be relocated.
	Path string
	Size int64
	// Existed reports that the content address was already present, which is
	// what makes a partial re-run cheap.
	Existed bool
}
