...
`cmd/server/internal/registry` resolves the backend named by the settings row **per call**. An unregistered name is an error, never a silent fallback. Backends are registered in `main.go` before settings load, so a bad row fails at startup. Currently wired: `sample` (every port but the thumbnail), `ffmpeg` (composer), `builtin` (thumbnail), `9router` (llm), `runware` (slide, thumbnail icons), `xtts` (tts). A fresh database is seeded to the `sample` preset, so a clean install runs locally and costs nothing.
...
`adapters/provider/sample` reads `<resources>/sample/`: `*.wav` narration, `img*.jpg` slides, `icon*.jpg` thumbnail tiles, `video.mp4` for the composer. The icons and the video are optional — they arrived after the rest, so a library without them still serves the others and only `Icons()`/`Video()` report the absence. Two of its backends read nothing: `llm.go` generates, because a canned outline could not answer the chapter count the DAG is built from, and `uploader.go` writes a receipt, because a publish has no artifact to serve. Its composer hands the same file back for every clip and for the final render, so fifty chapters cost one row and one file — ffmpeg is what to select when cuts, timing or burnt-in text need to be real. They are also the one sample that is transformed on the way in: an icon is square by the port's definition and the samples are 16:9, so they are centre-cropped and scaled to the requested size rather than reaching the renderer stretched.
...
`sample` is the one backend that answers nearly every port, and it does so two ways. Narration, slides and icons are read from disk; the LLM and the composer's stand-in generate, because words have no honest canned form — a blueprint must come back with the chapter count the DAG is built from — and a publish has no artifact at all. `seedOf` and `deterministic` in `llm.go` are what make the generated half reproducible, which is what lets content addressing dedupe a re-run. There is no separate mock backend: one local name is enough, and two would only raise the question of which to select.
