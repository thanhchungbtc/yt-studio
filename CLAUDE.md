...
Adapters are grouped by **whether the registry selects them by name**: `adapters/provider/*` is exactly the set a settings row can name, one directory per backend, mirroring the ports in `domain/provider`. `assetstore`, `eventbus` and `sqlite` sit at the root because nothing chooses them — they are constructed once in `main.go` and never resolved from a row. That is the line, not the capability served: `sample` answers three ports and `runware` two, so grouping by capability put multi-port backends nowhere and split the mock for a reason that applied to half the tree.
